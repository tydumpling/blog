...
Number.isNaN(Number.NaN / 9) // true
```
...
```js
(function (global) {
  const floor = Math.floor; const isFinite = global.isFinite
...
Number.EPSILON > 5.551115123125783e-17 // true
```
...
Math.cbrt = Math.cbrt || function (num) {
  const y = Math.abs(num) ** (1 / 3) // 取绝对值后，计算其立方根
  return num < 0 ? -y : y
}
```
...
```js
const a = 9n + 5n // 14n (加法)
const b = 11n - 6n // 5n (减法)

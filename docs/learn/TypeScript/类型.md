...
- 使用 js 不能自定义类型，比如下面的 name 我们想只允许是 daodao、data，但实际上是可以传递任何值的。
  ...
  function getTitle(name) {
  	return name === 'daodao' ? 'tydumpling' : 'data';
  }
...
可以将 typescript 中的类型理解为一类值的集合，比如 'daodao'、'tydumpling'都属于 string 类型集合。
...
```js
const fn = ['daodao.com', 'tydumpling'] // const fn: string[]
...
```js
const fn = ['daodao.com', 'tydumpling',100] // const fn:(string|number)[]
...
```js
let fn:any[] =['daodao.com','tydumpling',2010,true]
```

也可以使用泛型的方式设置 any 类型数组

```js
let fn:Array<any> =['daodao.com','tydumpling',2010,true]
```
...
console.log(push(fn, 'TS', 'tydumpling')); // [ 'daodao.com', 'TS', 'tydumpling' ]
```

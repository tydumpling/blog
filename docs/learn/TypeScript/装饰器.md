...
const ErrorDecorator: MethodDecorator = (target: Object, propertyKey: string | Symbol, descriptor: PropertyDescriptor): void => {
    ...
    descriptor.value = () => {
        try {
            ...
            //%c 表示 css 样式
            console.log(`%ctydumpling daodao.com，tydumpling`, "color:green; font-size:20px;");
...
const ErrorDecorator = (message: string, title: string = 'tydumpling') => <MethodDecorator>(target: Object, propertyKey: string | Symbol, descriptor: PropertyDescriptor): void => {
    ...
    descriptor.value = () => {
        try {
            ...
            console.log(`%c，${title || `tydumpling daodao.com`}`, "color:green; font-size:20px;");
...
class fn {
    ...
    public draw() {
        document.body.insertAdjacentHTML('beforeend', `<div style="width:200px;height:200px;background-color:${this.color}">daodao.com tydumpling</div>`)
    }
}

...
```js
import { effect, reactive, tarck, trigger } from './effect.js'
...
2. 判断新旧节点是否一样，一样则不需要更新节点，不一样则考虑以下情况：
   ...
   function processTextNode(v1, v2, container) {
     if (v1) {
       v2.el = v1.el
   ...
   function processElement(v1, v2, container) {
     if (v1) {
       v2.el = v1.el
   ...
   function patchProps(old, newProps, el) {
     if (old === newProps)
       return
   ...
   function patchChildren(v1, v2, el) {
